...
```{r}
corPlot(cor.mas5, minCor = 0.75)
corPlot(cor.rma, minCor = 0.85)
corPlot(cor.vsnrma, minCor = 0.80)
corPlot(cor.gcrma, minCor = 0.85)
corPlot(cor.plier, minCor = 0.50)
```
...
boxplot(log2(exprs(Daten.mas5)), range = 0)
boxplot(exprs(Daten.rma), range = 0)
boxplot(exprs(Daten.gcrma), range = 0)
boxplot(exprs(Daten.vsnrma), range = 0)
boxplot(exprs(Daten.plier), range = 0)
```
...
corPlot(cor.rma, minCor = 0.7)
corPlot(cor.gcrma, minCor = 0.78)
corPlot(cor.vsnrma, minCor = 0.78)
```
...
Erzeuge Heatmaps der Ergebnisse. Aufgrund der vielen signifikanten Unterschiede 
hier nicht.
...
volcano(res$`difference in means`, pval = res$adj.p.value, 
        ...
        alpha = 0.25, ylab = "-log10(adj.p.value)", 
        xlab = "log-fache Veränderung")
```
